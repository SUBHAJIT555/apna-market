...
const SingleItem = ({ item, removeItemFromCart }: SingleItemProps) => {
  ...
  return (
    <div className="flex items-start gap-3 rounded-xl border border-neutral-300 bg-white p-3.5">
      ...
      <div className="min-w-0 flex-1 pt-0.5">
        <h3 className="line-clamp-2 text-sm font-semibold leading-snug text-dark">
          {item.title}
        </h3>
        <div className="mt-1.5 flex flex-wrap items-center gap-2">
          ...
          {item.quantity > 1 && (
            <span className="text-xs text-dark-4">×{item.quantity}</span>
          )}
        </div>
      </div>
      ...
      <button
        ...
        className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-neutral-300 bg-white text-dark-4 transition-colors duration-200 hover:border-orange-600/40 hover:text-orange-600 active:scale-95"
      >
