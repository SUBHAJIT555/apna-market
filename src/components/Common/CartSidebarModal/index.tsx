...
const CartSidebarModal = () => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`modal-content absolute right-0 top-0 flex h-full w-full max-w-[420px] flex-col border-l border-neutral-300 bg-gray-1 shadow-[-8px_0_32px_rgba(25,25,25,0.08)] transition-transform duration-300 sm:max-w-[440px] ${
          isCartModalOpen ? "translate-x-0" : "translate-x-full"
        }`}
      >
        {/* Header */}
        <div className="shrink-0 border-b border-neutral-300 px-5 pb-5 pt-5 sm:px-6 sm:pt-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <span className="mb-3 inline-flex items-center gap-2 rounded-full border border-neutral-300 bg-white px-3 py-1 text-xs font-medium text-dark">
                <svg
                  width="14"
                  height="14"
                  viewBox="0 0 24 24"
            ...
            <button
              ...
              className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-neutral-300 bg-white text-dark-4 transition-colors duration-200 hover:border-neutral-400 hover:text-dark active:scale-95"
            >
        ...
        {/* Footer */}
        {cartItems.length > 0 && (
          <div className="shrink-0 border-t border-neutral-300 bg-gray-1 px-5 py-5 sm:px-6">
            <div className="mb-4 flex items-center justify-between gap-4 rounded-xl border border-neutral-300 bg-white px-4 py-3.5">
              <p className="text-sm font-medium text-dark">Subtotal</p>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Link
                onClick={() => closeCartModal()}
                href="/cart"
                className="inline-flex items-center justify-center rounded-xl border border-neutral-300 bg-white px-4 py-3 text-sm font-medium text-dark transition-colors duration-200 hover:border-neutral-400"
              >
                View Cart
              </Link>
              <Link
                onClick={() => closeCartModal()}
                href="/checkout"
                className="inline-flex items-center justify-center rounded-xl bg-green-600 px-4 py-3 text-sm font-semibold text-white transition-colors duration-200 hover:bg-green-700 active:scale-[0.98]"
              >
                Checkout
              </Link>
            </div>
