...
const Header = () => {
  ...
  const cartButton = (
    ...
    >
      <span className="relative inline-block">
        ...
        <span className="absolute -right-2 -top-2.5 flex h-4.5 w-4.5 items-center justify-center rounded-full bg-green-600 text-2xs font-medium text-white">
          {product.length}
        </span>
      </span>
      <div className="text-left">
        <span className="block text-2xs uppercase text-dark-4">cart</span>
  ...
  const menuToggleButton = (className = "") => (
    <button
      ...
      className={`flex shrink-0 items-center justify-center p-1 text-green-600 transition-colors hover:text-green-700 ${className}`}
    >
      {navigationOpen ? (
  ...
  const navigationMenu = (onNavigate?: () => void) => (
    <nav className="w-full">
      <ul className="flex flex-col gap-1 xl:flex-row xl:items-center xl:gap-6">
        {menuData.map((menuItem, i) =>
          menuItem.submenu ? (
            ...
            <li
              key={i}
              className="group relative before:absolute before:left-0 before:top-0 before:h-[3px] before:w-0 before:rounded-b-[3px] before:bg-blue before:ease-out before:duration-200 hover:before:w-full xl:before:top-0"
            >
              <Link
                href={menuItem.path}
                onClick={onNavigate}
                className={`flex rounded-lg px-3 py-2.5 text-custom-sm font-medium text-dark transition-colors hover:bg-gray-1 hover:text-green-600 xl:rounded-none xl:px-0 xl:py-0 xl:hover:bg-transparent ${
  ...
  return (
    <header
      className={`fixed left-0 top-0 z-9999 w-full border-b border-neutral-300/50 bg-gray-1 transition-all duration-300 ease-in-out lg:border-white/40 lg:bg-white/70 lg:backdrop-blur-md lg:backdrop-saturate-150 ${
        stickyMenu && "lg:bg-white/85 lg:shadow-md lg:shadow-black/5"
      }`}
    >
      <div className="mx-auto max-w-[1170px] px-4 sm:px-7.5 xl:px-0">
        ...
        <div className="relative py-3 lg:hidden">
          ...
          {navigationOpen && (
            <div className="absolute left-0 right-0 top-full z-[10000] mx-0 mt-2 max-h-[min(70vh,400px)] overflow-y-auto rounded-xl border border-neutral-300 bg-white p-5 shadow-[0_12px_40px_rgba(25,25,25,0.12)]">
              {navigationMenu(closeMobileNav)}
            </div>
          )}
        </div>
        ...
        >
          <div className="flex w-full flex-col items-center gap-5 sm:flex-row sm:justify-between sm:gap-10 xl:w-auto">
            ...
            <div className="hidden max-w-[475px] w-full lg:block" ref={searchRef}>
      ...
      <div className="relative hidden border-t border-gray-3/60 lg:block xl:static">
        <div className="mx-auto max-w-[1170px] px-4 sm:px-7.5 xl:px-0">
          <div className="flex items-center justify-between">
            ...
            <div
              className={`absolute left-4 right-4 top-full z-[10000] h-0 w-auto max-w-[calc(100%-2rem)] items-center justify-between invisible lg:flex xl:static xl:left-auto xl:right-auto xl:z-auto xl:h-auto xl:max-w-none xl:visible ${
                navigationOpen &&
                "lg:!visible lg:!h-auto lg:max-h-[min(70vh,400px)] lg:overflow-y-auto lg:rounded-xl lg:border lg:border-neutral-300 lg:bg-white lg:p-5 lg:shadow-[0_12px_40px_rgba(25,25,25,0.12)]"
              }`}
            >
              {navigationMenu(() => setNavigationOpen(false))}
            </div>
            {/* // <!--=== Main Nav End ===--> */}
